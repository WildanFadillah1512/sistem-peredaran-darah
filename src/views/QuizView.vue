<template>
  ...
  >
    ...
    >
      <!-- FIXED: Background score badge tidak transparan lagi -->
      <div class="flex justify-between items-center mb-4 px-2">
        ...
        <!-- FIXED: Warna solid 100%, tidak ada opacity sama sekali -->
        <div
      ...
      <transition name="bounce-switch" mode="out-in">
        ...
        >
          <div class="relative w-full z-20 mt-8">
            ...
            >
              <div
                class="text-[5rem] md:text-[6rem] filter drop-shadow-xl animate-float-y select-none"
              >
                {{ mascotExpression }}
              </div>
            </div>
            ...
            <div
              class="bg-white/95 backdrop-blur-xl border-[6px] border-white shadow-[0_8px_0_rgba(0,0,0,0.1)] rounded-[3rem] p-6 pt-16 md:p-10 min-h-[220px] flex flex-col items-center justify-center text-center relative overflow-hidden group"
            >
              <button
                @click.stop="playQuestionAudio"
                class="absolute top-4 right-4 bg-blue-100 text-blue-500 p-2 rounded-full hover:bg-blue-200 transition-colors"
              >
                <Volume2 class="w-6 h-6" />
              </button>

              <h2
                class="text-xl md:text-3xl font-black text-slate-700 leading-snug relative z-10"
              >
          ...
          <div
            class="relative h-14 flex items-center justify-center -my-7 z-30 pointer-events-none"
          >
            <div
              class="w-20 h-20 bg-white rounded-full flex items-center justify-center shadow-lg border-[6px] border-white ring-4 ring-opacity-20"
              :class="`ring-${card.color.split('-')[1]}-400`"
            >
              <div
                class="w-full h-full rounded-full flex items-center justify-center font-black text-2xl text-white shadow-inner"
                :class="card.color"
              >
          ...
          <div class="grid grid-cols-2 gap-3 md:gap-4 mt-10 z-10 w-full pb-4">
            <button
              ...
              :disabled="isAnswered"
              class="relative group h-auto min-h-[80px] md:h-24 rounded-[1.5rem] md:rounded-[2rem] border-b-[4px] md:border-b-[6px] transition-all duration-200 transform active:scale-95 flex items-center overflow-hidden shadow-sm hover:brightness-105 px-2 py-2 md:px-6"
              :class="getOptionClass(index)"
            >
              <div
                class="w-8 h-8 md:w-12 md:h-12 rounded-full flex-shrink-0 flex items-center justify-center font-black text-sm md:text-xl shadow-sm border-2 border-white/30 mr-2 md:mr-4"
              ...
              <span
                class="text-xs md:text-xl font-bold text-left flex-1 leading-tight tracking-tight break-words"
              >
                {{ option }}
              </span>

              <span
                v-if="isAnswered && index === currentQuestion.correctIndex"
                class="absolute right-1 md:right-4 text-2xl md:text-4xl animate-pop-in"
                >🎉</span
              >
              <span
                v-if="
                ...
                "
                class="absolute right-1 md:right-4 text-2xl md:text-4xl animate-shake"
                >🙈</span
              >
            </button>
          </div>
        ...
        >
          <div
            class="bg-white border-[6px] border-white shadow-[0_10px_0_rgba(0,0,0,0.1)] rounded-[3rem] p-8 md:p-12 text-center w-full max-w-2xl transform transition-all hover:scale-[1.02]"
          >
            <div class="mb-6 relative inline-block">
              <div class="text-[8rem] animate-bounce-slow">
                {{ resultEmoji }}
              </div>
              <div
                class="absolute -bottom-2 -right-2 bg-yellow-400 text-yellow-900 font-black px-4 py-1 rounded-full text-lg shadow-sm border-2 border-white rotate-12"
              >
            ...
            <h2
              class="text-3xl md:text-5xl font-black text-slate-800 mb-2 tracking-tight"
            >
              {{ resultTitle }}
            </h2>
            <p class="text-slate-500 font-bold mb-8 text-xl">
              Kamu dapat
              <span class="text-yellow-500 text-2xl">⭐ {{ score * 10 }}</span>
              poin!
            </p>
            ...
            <div class="flex flex-col md:flex-row gap-4 justify-center w-full">
              <button
                @click="restartQuiz"
                class="flex-1 bg-slate-100 text-slate-600 py-4 rounded-2xl font-black text-lg border-b-4 border-slate-200 hover:bg-slate-200 hover:border-slate-300 active:translate-y-1 active:border-b-0 transition-all"
              >
                🔄 MAIN LAGI
              </button>
              <button
                @click="finishQuiz"
                class="flex-1 bg-green-500 text-white py-4 rounded-2xl font-black text-lg border-b-4 border-green-700 shadow-lg hover:bg-green-400 active:translate-y-1 active:border-b-0 active:shadow-none transition-all"
              >
                🚀 LANJUT
              </button>
            </div>
      ...
      <transition name="fade">
        <div
          v-if="showConfetti"
          class="absolute inset-0 z-50 pointer-events-none flex items-center justify-center overflow-hidden"
        >
          ...
          <div
            class="bg-white/90 backdrop-blur px-8 py-4 rounded-3xl shadow-2xl border-4 border-green-200 transform scale-125 animate-pop-in z-50"
          >
...
const getLabelClass = (index) => {
  if (!isAnswered.value)
    return "bg-slate-100 text-slate-400 group-hover:bg-yellow-400 group-hover:text-yellow-900";
  if (index === currentQuestion.value.correctIndex)
...
.bounce-switch-enter-active {
  animation: popIn 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275);
}
.bounce-switch-leave-active {
